...
const Root = ({ children, defaultValue, variant = 'default', className }: TabsRootProps) => {
...
const List = ({ children, className }: TabsListProps) => {
  ...
  const baseStyles = css({
    ...
    bgColor: variant === 'enclosed' ? 'bg.tertiary' : 'transparent',
...
const Trigger = ({ children, value, className, asChild }: TabsTriggerProps) => {
  ...
  const baseStyles = css({
    ...
    flex: '1',
    _hover: {
      color: isActive ? 'text.bright' : 'text.bright',
